...
{
	char *tmpBuf = buf; //This is a copy of the character buffer that's gonna be used in this function

	int isSpace; //this is going to be a flag to mark the first character of the input text to be a space or not
	if(isspace(buf[0])){
		isSpace = 1;
	}
	else{
	...
	//tackle the case where the first character is a space
	if(isLetter == 0)
	{
		
	}
	int tempCount = 0; //will be used to loop through the buffer
	while(isspace(buf[tempCount])){
		tempCount++;
	}
